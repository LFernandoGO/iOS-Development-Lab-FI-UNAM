...
class EmojiCollectionViewController: UICollectionViewController {
        ...
        didSet{
            if let layout = layout[activeLayout]{
                ...
                collectionView.setCollectionViewLayout(layout, animated: true){_ in
                    ...
                    case .grid:
                        self.layoutButton.image = UIImage(systemName: "rectanble.grid.1x2")
                    case .column:
    ...
    func generateColumnarLayout() -> UICollectionViewLayout{
        ...
        let group = NSCollectionLayoutGroup.horizontal(layoutSize: NSCollectionLayoutSize(widthDimension: .fractionalWidth(1), heightDimension: .fractionalHeight(1/6)), subitem: item, count: 1)
        //group.interItemSpacing = .fixed(padding)
        group.contentInsets = NSDirectionalEdgeInsets(top: 0, leading: padding, bottom: 0, trailing: padding)
